<?php
    class extra{
        ...
        function input( $input ){
            ...
            switch( $type ){
                case 'text' :{
                    ...
                    $result .= '<div class="lvisible ' . $lvisible . '">'  . $before . $value . $after . '</div>';
                    ...
                    $result .= '<input type="text" id="' . $id . '" class="' . $classes . '" name="' . $name . '" value="' . $value . '"/>';
                    $result .= '</div>';
                    $result .= '<div class="clear"></div>';
                    $result .= '</div>';
                ...
                case 'textarea' :{
                    $result .= '<p class="lvisible ' . $lvisible . '">' . $before . $value . $after . '</p>';
                    $result .= '<div class="fvisible field ' . $fvisible . '">';
                    $result .= '<div  class="label">';
                    $result .= $label;
                    $result .= '</div>';
                    ...
                    $result .= '<div  class="input">';
                    $result .= '<textarea  id="' . $id . '" class="' . $classes . '" name="' . $name . '">' . $value . '</textarea>';
                    $result .= '</div>';
                    $result .= '<div class="clear"></div>';
                    $result .= '</div>';
                    break;
        ...
        function sort( $group = null , $side = null , $data = array() ){
            if( empty( $group ) ){
                $group  = isset( $_POST['group'] ) && strlen( $_POST['group'] ) ? trim( $_POST['group'] ) : exit;
                $side   = isset( $_POST['side'] ) && strlen( $_POST['side'] ) ? trim( $_POST['side'] ) : exit;
                $data   = isset( $_POST['data'] ) ? $_POST['data'] : exit;
            ...
            foreach($data as $value ){
                if( $value['name'] == $name.'[]' ){
                    $new_options[ $index ] =  $op_group[ $side ][$value['value']];
                    $new_options[ $index ][ $name ] = $index;
                    unset( $op_group[ $side ][$value['value']] );
                    $index++;
                }
            }
            
            $new_index = count( $new_options );
            foreach( $op_group[ $side ] as $value){
