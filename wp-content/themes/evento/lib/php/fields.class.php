<?php
    class fields {
        function layout( $field ){
            ...
            $hint = isset( $hint ) ? '<span class="generic-hint">' . $hint . '</span>': '' ;
        ...
        function field( $field ){
            ...
            switch( $type  ){
                    ...
                    if( is_array( $value ) && !empty( $value ) ){
                        ...
                        foreach( $value  as $index => $icon ){
                            if( $i == 0 ){
                                ...
                                $result .= '<input type="radio" value="' . get_template_directory_uri() . '/lib/images/' . $path . $icon . '" name="' . $name . '" class="generic-record  hidden '  . $fclasses .  ' ' . $classes . '" ' . $id . ' ' . $s . '>';
                                $result .= '</div>';
                                $result .= '<img ' . self::action( $action , 'radio-icon' ) . ' title="' . $icon . '" class="pattern-texture ' . $index. ' '. $sclasses .'" alt="' . $icon . '" src="' . get_template_directory_uri() . '/lib/images/' . $path . $icon . '" />';
                            $i++;
                            if( $i % $in_row == 0 ){
                                $i = 0;
                ...
                case 'logic-radio' : {
                    ...
                    $result  = '<input type="radio" value="yes" name="' . $name . '" class="generic-record  '  . $fclasses .  ' ' . $classes . '" ' . $id . ' ' . $c1 . ' /> ' . __( 'Yes' , 'cosmotheme' ) . '&nbsp;&nbsp;&nbsp;';
                    $result .= '<input type="radio" value="no" name="' . $name . '" class="generic-record  '  . $fclasses .  ' ' . $classes . '" ' . $id . ' ' . $c2 . '/> ' . __( 'No' , 'cosmotheme' );
                    break;
                }
                /* single type records */
                case 'hidden' : {
                ...
                case 'button' : {
                    $result .= '<input type="button" name="' . $name . '" value="' . $value . '" class="generic-record-button  button-primary  ' . $classes . '" ' . $id . ' ' . self::action( $action , 'button' ) . ' />';
                    break;
                }
        ...
        function action( $action , $type ){
            ...
            switch( $type ){
                ...
                case 'radio-icon' : {
                    $result = 'onclick="javascript:act.radio_icon(\'' . $action['group'] . '\' , \'' . $action['index'] . '\' );"';
                    break;
                }
                case 'textarea' : {
                    $result = 'onkeyup="javascript:' . $action . ';"';
                ...
                case 'upload-id' : {
                    $result = 'onclick="javascript:act.upload_id(\'' . $action['group'] . '\' , \'' . $action['topic'] . '\' , \''.$action['index'].'\' );"';
                    break;
                }
            }
