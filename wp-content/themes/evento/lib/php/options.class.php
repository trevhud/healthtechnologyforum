<?php
    class options{
        ...
        static function cosmothemes__checkout(){
            self::CallMenu( 'cosmothemes__checkout' );
        }
...
<?php
		...
		function get_taxonomy( $post_type , $taxonomy_type )
			{
				switch( $post_type ){
					case 'conference': $option_name="conf_slug"; break;
					case 'exhibitor': $option_name="exhib_slug"; break;
					case 'sponsor': $option_name="sponsor_slug"; break;
					case 'speaker': $option_name="speaker_slug"; break;
					case 'presentation': $option_name="present_slug"; break;
					default: return false;
				}
				$option = self::get_value( 'blog_post' , $option_name );
				$result = ( strlen( $option ) > 0 ) ? $option : $post_type;
				return ( $taxonomy_type == 'category' ) ? $result.'-category' : $result.'-tag';
			}
