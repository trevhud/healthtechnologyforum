<?php
    class widget_exhibitors extends WP_Widget{
        function widget_exhibitors() {
            $options = array( 'classname' => 'exhibitors  images', 'description' => __('Display list of Exhibitors from a Conference' , 'cosmotheme' ) );
            parent::WP_Widget( false , _TN_ . ' : ' . __( 'Conference Exhibitors' , 'cosmotheme' )  , $options );
        ...
        function form($instance) {
            if( isset($instance['title']) ){
                ...
                $title = __( 'Conference exhibitors' , 'cosmotheme' );
            }
		...
		<p>
			...
			<select class="widefat" id="<?php echo $this->get_field_id('category');?>" name="<?php echo $this->get_field_name('category');?>">
				...
				<?php
					$terms=get_terms( options::get_taxonomy( 'exhibitor' , 'category' ) );
        ...
        <!-- field for set limit  -->
        <p>
          <label for="<?php echo $this -> get_field_id('nr'); ?>"><?php _e( 'Number exhibitors to display' , 'cosmotheme' ); ?>:</label>
        ...
        function widget($args, $instance) {
            ...
            if( (int) $conf_id > 0 ){
                echo conference::exhibitors( $conf_id , $nr , $excerpt , $category );
                /* read more link */
                if( (int) $page_id > 0 ){
