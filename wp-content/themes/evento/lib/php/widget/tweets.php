<?php
    class widget_tweets extends WP_Widget {
        ...
        <p>
        	...
        	<input type="checkbox" id="<?php echo $this->get_field_id( 'dynamic' ); ?>"  <?php checked( $instance[ 'dynamic' ], true ); ?>  name="<?php echo $this->get_field_name( 'dynamic' ); ?>"  value="1" />
        </p>
        <?php
        }
  ...
  class tweets_new{
		...
		function get_tweets( $username , $number , $classes = '' , $before = "<div class='tweet_item'><p>" , $after = "</p></div>", $static_class = '' ){
			...
			*/
			...
			$cacheTime = options::get_value( 'social' , 'twitter_update_intervat' ); // Time in minutes between updates.
			...
			if(false === ($tweets = get_transient($transName) ) ||  get_transient($transName) == '') :    
				...
				if( !is_wp_error( $json ) ) {
					...
					if(is_array($tweets) && count($tweets) > 0){
						...
						set_transient($transName.'_old', $tweets, 500 * 60 * $cacheTime);	
					}
